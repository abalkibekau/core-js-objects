...
function makeWord(lettersObject) {
  ...
  for (let i = 0; i < keys.length; i += 1) {
    const positions = lettersObject[keys[i]];
    for (let j = 0; j < positions.length; j += 1) {
      res[positions[j]] = keys[i];
    }
  }
  return res.join('');
}
